...
public abstract class LinkCollectionCompiler<T> {
    ...
    private void manageOutputPorts(Set<PortNumber> outPorts,
        ...
        for (PortNumber outPort : outPorts) {
            ...
            if (!filteredEgressPoint.isPresent()) {
                ...
                /*
                 * We add the instruction necessary to the meter
                 */

                treatmentBuilder.setIpDscp((byte) 0xa);

                try {
                    if (intent.treatment().metered() != null) {
                        if(intent.treatment().metered().meterId().equals(MeterId.meterId(1L))){
                            log.info("Dentro if meters");
                            outPorts.forEach(portNumber -> {
                                treatmentBuilder.meter(MeterId.meterId(portNumber.toLong()));
                            });
                        }
                        else {
                            log.info("Dentro if meter");
                            outPorts.forEach(portNumber -> {
                                treatmentBuilder.meter(ID_Meter.buildMeterId(intent.treatment().metered().meterId(), portNumber));
                            });
                        }
                 ...
                 */
                try {
                    if (intent.treatment().tableTransition() != null) {
                        if (intent.treatment().tableTransition() != null) {
                            treatmentBuilder.transition(intent.treatment().tableTransition().tableId());
                        }
                    }
                    else{
                        log.info("Transition null");
                        treatmentBuilder.setOutput(outPort);
                    }
                }
                catch(IllegalArgumentException e)
                {
                    log.info("No transition ");
                }
                /*
    ...
    private void manageMpIntent(TrafficSelector.Builder selectorBuilder,
        ...
        //selectorBuilder.matchIPDscp((byte) 10);
        treatmentBuilder.setIpDscp((byte) 0xa);
        try {
            if (intent.treatment().metered() != null) {
                if(intent.treatment().metered().meterId().equals(MeterId.meterId(1L))){
