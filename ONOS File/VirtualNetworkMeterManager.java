...
public class VirtualNetworkMeterManager
    ...
    @Override
    public Meter submitWithId(MeterRequest request, MeterId meterId) {
        if (isNullOrEmpty(getMeter(request.deviceId(), meterId).toString()))
        {
            log.info("Meter id {} already used in device {}",meterId,request.deviceId());
            return null;
        }
